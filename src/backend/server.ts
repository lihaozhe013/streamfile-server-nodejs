...
import { ensureRuntimeDirectories, getLocalIp, loadRuntimeConfig } from '@/config';
...
const server = app.listen(runtime.server.port, runtime.server.host, () => {
  const host = runtime.server.host === '0.0.0.0' ? getLocalIp() : runtime.server.host;
  void appendDebugLog(
...
const shutdown = (signal: string) => {
  ...
  shuttingDown = true;
  void appendDebugLog(runtime.paths.dataRoot, `[backend_server] Received ${signal}, shutting down`);
  server.close((error) => {
    if (error) {
      void appendDebugLog(
        runtime.paths.dataRoot,
