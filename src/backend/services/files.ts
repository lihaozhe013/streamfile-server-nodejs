...
export function isWithinDirectory(baseDirectory: string, candidatePath: string): boolean {
...
export function resolveWithinDirectory(baseDirectory: string, relativePath: string): string | null {
  const candidate = path.resolve(baseDirectory, normalizeRelativePath(relativePath));
...
export function isIncomingPath(paths: RuntimePaths, candidatePath: string): boolean {
  return isWithinDirectory(paths.incomingDir, candidatePath);
}

export function isPrivatePath(paths: RuntimePaths, candidatePath: string): boolean {
