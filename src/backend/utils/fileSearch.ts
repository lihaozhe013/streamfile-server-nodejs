...
export const handleSearchRequest = (req: Request, res: Response) => {
  ...
  const fileName =
    (req.query.q as string) || (req.params as Record<string, string>)[0];
  const currentDir =
    (req.query.dir as string) ||
    ((req.params as Record<string, string>)[1] || '').toString();

  if (!fileName) {
    return res.status(400).json({ error: 'file_name parameter is required' });
  }

  const safeCurrentDir = path
  ...
  try {
    ...
    const filteredFiles = files.filter(
      (file: { path: string; full_file_name: string }) => {
    ...
    const resultFiles = filteredFiles.map(
      (file: { full_file_name: string; path: string }) => {
        return {
